  ...
  loadActivePlan(): void {
    ...
    this.workoutService.getMyWorkout().subscribe({
      next: (r) => {
        ...
        const plan = r.data;
        this.activePlanId.set(plan?.workoutPlanId ?? null);
      ...
      error: () => {
        ...
        this.notify.error('Failed to load your workout plan');
      },
    });
  }

  save(): void {
    const workoutPlanId = this.activePlanId();
    if (!workoutPlanId) {
      this.notify.error('No active workout plan is assigned to your account.');
      return;
    }
