...
export interface AssignWorkoutPlanDialogData {
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>
      {{ data.planName ? 'Assign "' + data.planName + '"' : 'Assign workout plan' }}
    </h2>
    <mat-dialog-content>
      @if (loadingMembers) {
        ...
        @if (!data.workoutPlanId) {
          <mat-form-field appearance="outline" class="full">
            ...
            <mat-select formControlName="workoutPlanId" required>
              @for (p of plans; track p.workoutPlanId) {
                <mat-option [value]="p.workoutPlanId">{{ p.planName }}</mat-option>
              }
            </mat-select>
          </mat-form-field>
        }
...
export class AssignWorkoutPlanDialogComponent {
  readonly data = inject<AssignWorkoutPlanDialogData>(MAT_DIALOG_DATA);
  ...
  constructor() {
    if (!this.data.workoutPlanId) {
      this.workout.getPlans(false).subscribe({
        next: (res) => {
  ...
  submit(): void {
    ...
    this.workout.assignMany(payload, memberIds).subscribe({
      next: ({ succeeded, failed }) => {
        this.saving = false;
        if (failed === 0) {
          this.notify.success(
            memberIds.length === 1 ? 'Workout plan assigned' : `Workout plan assigned to ${succeeded} members`
          );
          this.ref.close(true);
          return;
        }
